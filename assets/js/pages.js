(() => {
  ...
  const snapPages = [...document.querySelectorAll('main > .about-snap-page')];
  if (snapPages.length > 1) {
    ...
    const updateIndex = () => {
      ...
      for (let i = snapPages.length - 1; i >= 0; i--) {
        if (scrollPos >= snapPages[i].offsetTop) {
          currentPageIndex = i;
          break;
        }
      }
    ...
    const goToPage = (index) => {
      if (index < 0 || index >= snapPages.length || isAnimating) return;
      isAnimating = true;
      currentPageIndex = index;
      snapPages[index].scrollIntoView({ behavior: reduceMotion ? 'auto' : 'smooth', block: 'start' });
      setTimeout(() => {
        isAnimating = false;
        updateIndex();
      }, 550);
    ...
    window.addEventListener('wheel', (e) => {
      ...
      if (delta > 0 && currentPageIndex < snapPages.length - 1) {
        e.preventDefault();
    ...
    window.addEventListener('keydown', (e) => {
      ...
      if (['ArrowDown', 'PageDown', ' '].includes(e.key) && currentPageIndex < snapPages.length - 1) {
        e.preventDefault();
        goToPage(currentPageIndex + 1);
